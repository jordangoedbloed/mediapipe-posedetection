...
//   minPerLabelEl.textContent = String(config.minSamplesPerLabel);

//   autoGoodBtn.addEventListener('click', () => startCountdown('goede_houding'));
//   autoBadBtn.addEventListener('click', () => startCountdown('slechte_houding'));
//   cancelAutoBtn.addEventListener('click', cancelCountdown);
//   trainBtn.addEventListener('click', trainModel);
//   accuracyBtn.addEventListener('click', calculateAccuracy);
//   exportBtn.addEventListener('click', exportToJSON);
//   importBtn.addEventListener('click', importFromJSON);
//   clearBtn.addEventListener('click', clearData);

//   setButtonsDuringCountdown(false);
...
//     if (result.label === 'goede_houding') {
//       liveBadge.textContent = 'Goed';
//       setBadgeState('good');
//     } else if (result.label === 'slechte_houding') {
...
//   setButtonsDuringCountdown(true);
//   setFeedback(`Aftellen: ${countdownRemaining}... blijf stil zitten.`, 'neutral');
...
//       if (!currentLandmarks) {
//         setButtonsDuringCountdown(false);
//         setFeedback('Geen pose gevonden op het moment van opslaan.', 'bad');
//         return;
//       }
...
//   setButtonsDuringCountdown(false);
//   setFeedback('Aftellen gestopt.', 'neutral');
// }

// function setButtonsDuringCountdown(isCounting) {
//   cancelAutoBtn.disabled = !isCounting;
//   autoGoodBtn.disabled = isCounting;
//   autoBadBtn.disabled = isCounting;

//   trainBtn.disabled = isCounting;
//   accuracyBtn.disabled = isCounting;
//   exportBtn.disabled = isCounting;
//   importBtn.disabled = isCounting;
//   clearBtn.disabled = isCounting;
// }
...
//   const friendly = label === 'goede_houding' ? 'goede houding' : 'slechte houding';
//   setFeedback(`Sample opgeslagen: ${friendly}. (totaal ${savedPoses.length})`, 'good');
// }

// function hasEnoughTrainingData() {
//   const good = savedPoses.filter(p => p.label === 'goede_houding').length;
//   const bad = savedPoses.filter(p => p.label === 'slechte_houding').length;
...
//   setFeedback(`Model getraind met ${savedPoses.length} samples.`, 'good');
// }
...
//   if (numLabels === 0) {
//     setFeedback('Train eerst het model voordat je accuracy berekent.', 'bad');
//     return;
//   }

//   if (savedPoses.length === 0) {
//     setFeedback('Geen data om te testen.', 'bad');
//     return;
//   }
...
//   for (const p of savedPoses) {
//     try {
//       const res = await knnClassifier.classify(p.pose.flat());
//       if (res.label === p.label) correct++;
//     } catch (error) {
//       console.error('Classificatie fout:', error);
//     }
//   }

//   const acc = (correct / savedPoses.length) * 100;
//   setFeedback(`Accuracy (op je trainingsdata): ${acc.toFixed(1)}%`, 'neutral');
// }
...
//         item &&
//         (item.label === 'goede_houding' || item.label === 'slechte_houding') &&
...
//   countGoodEl.textContent = String(good);
//   countBadEl.textContent = String(bad);
//   countTotalEl.textContent = String(savedPoses.length);
// }
...
function init() {
  ...
  autoGoodBtn.addEventListener('click', () => startCountdown(LABEL_GOOD));
  autoBadBtn.addEventListener('click', () => startCountdown(LABEL_BAD));
  cancelAutoBtn.addEventListener('click', cancelCountdown);
  trainBtn.addEventListener('click', trainModel);
...
function saveSample(label) {
  ...
  const friendly = label === LABEL_GOOD ? 'goede houding' : 'slechte houding';
  setFeedback(`Sample opgeslagen: ${friendly}. (totaal ${savedPoses.length})`, 'good');
}
